...
> 当前实现说明: `paper_profile_gate.py` 是 probe、pilot、full 唯一的主张门禁实现。
> `pilot_paper_gate.py` 与 `full_paper_result_checker.py` 只保留旧 API、CLI、文件名和
> 决策字段兼容转发, 不再执行第二套机制检查。本文后续出现的
> `full_paper_result_checker` 均应理解为“公共 gate 的 full_paper 兼容接口”。

```text
让 Codex 在后续工程实现时, 能按固定接口逐步实现 probe_paper gate、pilot_paper gate、full_paper_result_checker、modern baseline runner、adaptive attack runner、statistical CI reporter、reviewer evidence index builder 和轻量判定组件。
```
...
| 组件 | 作用 | 产物性质 |
|---|---|---|
| `paper_profile_gate` | 三个 profile 共用的 gate builder; 在 probe_paper 运行中同时写出通用 artifact 与 probe 专属 artifact | gate builder |
| `probe_paper_gate` | `paper_profile_gate` 在 probe_paper 配置下同次生成的规范门禁 artifact, 判断 FPR=0.1 下三层结论与相对优势是否成立 | gate decision |
...
| `paper_profile_gate` | 参数化检查 probe、pilot、full 的同一组 records、tables、figures、reports 与三层主张; 仅 FPR 和样本统计规模不同 | profile-specific gate decision |
| `reviewer_evidence_index_builder` | 将审稿问题映射到 governed artifacts, 由 submission_freeze 消费 | governed report |
| `stage_transition_decision` | 生成阶段明确的跳转判定, 包括 probe_paper -> pilot_paper、pilot_paper -> full_paper、full_paper -> submission_freeze; 该判定只能在 source gate 已 PASS 后生成, 不得作为 source gate 自身的 PASS 前置 | lightweight decision |
...
experiments/generative_video_model_probe/pilot_paper_gate.py  # 仅保留公共 gate 兼容转发
scripts/check_results/full_paper_result_checker.py  # 仅保留公共 gate 兼容转发
...
evaluation/statistics/clustered_inference.py
experiments/generative_video_model_probe/statistical_confidence_interval.py
experiments/generative_video_model_probe/reviewer_evidence_index.py
...
tests/functional/test_clustered_statistics.py
...
`paper_profile_gate` 不读取 `probe_paper_to_pilot_paper_transition_decision`, 因为该跳转判定必须在 `paper_profile_gate_decision == PASS` 之后才能生成。这样可以避免“probe_paper PASS 依赖跳转判定, 跳转判定又依赖 probe_paper PASS”的循环依赖。`probe_paper_to_pilot_paper_transition_decision` 只能在后续 `probe_paper_gate_decision == PASS` 后生成, 不能作为 probe_paper 的输入。
...
figures/paper_profile_gate_figure.json
...
其中 `figures/paper_profile_gate_figure.json` 可以是最小诊断图 manifest, 但必须由 records 和 `paper_profile_gate_decision.json` 重建, 不能手工填写。当前实现路径为 `experiments/generative_video_model_probe/probe_paper_artifact_package.py`。

### 3.3 PASS 条件

只有同时满足以下条件, `paper_profile_gate` 才能写出 PASS。在 probe_paper 配置下, 同一次执行还会写出 `probe_paper_gate_decision == PASS`; 随后生成跳转判定才能进入 pilot_paper。该 PASS 支撑 FPR=0.1 下的完整结论, 但不允许外推为 pilot_paper 或 full_paper 结论:

```text
target_fpr == configs/protocol/probe_paper_generative_probe.json::target_fpr
all_required_phase_decisions_exist
all_required_phase_decisions_passed
...
replay_and_authenticated_sketch_gate_passed
confidence_interval_report_ready
artifact_rebuild_dry_run_ready
claim_audit_report_ready
package_manifest_ready
```

`probe_paper` 通过后可以声明其预注册协议下的 `TPR@FPR=0.1`、Claim-1、Claim-2 与 Claim-3 结论。该结论是样本规模较小、置信区间较宽的完整论文结论, 不能外推为 `pilot_paper` 的 FPR=0.01 或 `full_paper` 的 FPR=0.001 结果; 后两者仍需各自的独立门禁、CI、claim audit 和 artifact rebuild 全部通过。

`paper_profile_gate_decision == PASS` 落盘后, 才允许运行 `stage_transition_decision` 生成 `probe_paper_to_pilot_paper_transition_decision`。该跳转判定只决定是否允许进入 `pilot_paper`, 不反向参与 `probe_paper` 自身 PASS 计算。

## 4. probe_paper_gate 接口规范

`probe_paper` 是 target_fpr=0.1 的完整论文闭合层。它使用与后续 profile 同构的 attack、baseline、消融、统计、图表和打包协议, 但采用较少独立视频。`probe_paper_gate` 不是第二次运行, 而是 shared builder 同次写出的规范 artifact。它通过后可报告 FPR=0.1 下的三层结论, 并且只能继续进入 `pilot_paper`, 不能直接进入 `full_paper`。

### 4.1 输入

必须输入当前 `probe_paper` run_root 下的 generation、quality、runtime attack、runtime detection、external baseline、fair calibration、formal comparison、difference interval、internal ablation、adaptive attack、low FPR blocking、motion consistency 和 artifact rebuild records, 并读取上游:
...
当前实现复用 `experiments/generative_video_model_probe/paper_profile_gate.py` 的公共 gate builder, 但通过 `configs/protocol/probe_paper_generative_probe.json` 将 `paper_result_level`、`stage_id`、样本规模和 claim 边界切换到 `probe_paper`。

### 4.3 PASS 条件

`probe_paper_gate` 与 shared builder 使用同一批输入和同一组完整机制要求, 并额外固定以下 profile 身份与优势条件:

```text
paper_result_level == probe_paper
target_fpr == configs/protocol/probe_paper_generative_probe.json::target_fpr
paper_profile_gate_decision == PASS
probe_paper_profile_generation_records_ready
probe_paper_target_fpr_0_1_sstw_advantage_claim_supported
```

`probe_paper_gate_decision == PASS` 后, 才允许运行 `stage_transition_decision` 生成 `probe_paper_to_pilot_paper_transition_decision`。该判定由 `pilot_paper_gate` 消费, 不反向参与 `probe_paper` 自身 PASS 计算。

## 5. pilot_paper_gate 接口规范

### 5.1 输入

`pilot_paper_gate` 读取已经落盘的 pilot_paper records 与前置 decision artifacts。该 gate 允许读取 pilot_paper 的 held-out test split, 因为它本身就是小规模完整协议执行阶段; 但它不得读取 full_paper held-out test 结果, 也不得把 probe_paper 或 pilot_paper 结果外推为 full_paper 主表。
...
## 6. modern_external_baseline_runner 接口规范
...
### 6.4 external_baseline_self_containment_decision 输出
...
## 7. flow_specific_adaptive_attack_runner 接口规范
...
## 8. statistical_confidence_interval_reporter 接口规范
...
## 9. full_paper_result_checker 兼容接口规范

`full_paper_result_checker` 必须读取 `configs/protocol/full_paper_generative_probe.json`, 验证其已匹配公共机制契约, 然后原样调用 `paper_profile_gate`。它只增加历史字段和文件名别名, 不得拥有独立样本、攻击、baseline 或 claim 检查。
...
## 10. reviewer_evidence_index_builder 接口规范

`reviewer_evidence_index_builder` 必须在当前 profile 的最终 gate decision 为 PASS 后运行。它只索引已经由 gate 确认可使用的 records、tables、artifacts 和 reports, 并计算文件内容摘要; 不存在以范围收缩状态绕过三层 claim 的入口。
...
### 11.1 stage_transition_decision
...
artifacts/probe_paper_to_pilot_paper_transition_decision.json
artifacts/pilot_paper_to_full_paper_transition_decision.json
...
```text
paper_profile_gate 不读取 probe_paper_to_pilot_paper_transition_decision
probe_paper_gate 不读取 probe_paper_to_pilot_paper_transition_decision
pilot_paper 公共 gate 不读取 transition decision, 由外层 stage 控制是否允许启动
full_paper 公共 gate 不读取 transition decision, 由外层 stage 控制是否允许启动
submission_package_freeze 读取 full_paper_to_submission_freeze_transition_decision
```

该判定用于防止从较低阶段直接跳到不合法阶段。transition 属于公共 gate 外层的运行许可; probe、pilot、full 的正式 claim 均由同一个参数化 `paper_profile_gate` 决定。
...
### 11.3 data_split_and_leakage_guard
...
6. replay_and_authenticated_sketch_gate, 失败时阻断当前 paper profile
7. statistical_confidence_interval_reporter
8. paper_profile_gate
9. probe_paper_to_pilot_paper_transition_decision
10. probe_paper_gate
11. probe_paper_to_pilot_paper_transition_decision
12. pilot_paper_gate
13. pilot_paper_to_full_paper_transition_decision
14. full_paper_result_checker
15. reviewer_evidence_index_builder
16. full_paper_to_submission_freeze_transition_decision
...
## 15. external_baseline adapter comparison gate
...
### 15.1 external_baseline 正式 claim 升级条件
