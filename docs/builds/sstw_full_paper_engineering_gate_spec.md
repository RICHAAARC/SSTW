...
| 组件 | 作用 | 产物性质 |
|---|---|---|
| `validation_scale_readiness_gate` | 进入 pilot_paper 前的 target_fpr=0.1 小样本完整协议论文主张候选门禁, 验证完整论文产物链路是否可生成且 SSTW 是否具备 fpr=0.1 有效性与优势证据 | gate decision |
...
`validation_scale` 的正式定义是“target_fpr=0.1 小样本完整协议论文主张候选验证”。它不是 full_paper 效果证明, 也不是只检查单个 runner 是否能启动的 smoke test。该 gate 的功能是作为 pilot_paper 前的完整协议打通层, 在 validation_scale protocol config 指定的 `target_fpr=0.1` 小样本口径下提前验证 `pilot_paper` 和 `full_paper` 会需要的产物类型是否都能由本项目自动生成, 同时要求 SSTW 本方法 measured_formal、5 个现代 external baseline 同协议比较、差值置信区间、内部消融、低 FPR 阻断记录和 motion consistency 解释均已闭合。
...
原因是 `validation_scale` 是进入 `pilot_paper` 前的 target_fpr=0.1 小样本完整协议论文主张候选层。所有 paper 相关机制必须先在 validation_scale protocol config 指定的 target_fpr 小样本口径下产出 governed records、tables、figures、reports、manifests 和 claim audit, 并证明 SSTW 在 fpr=0.1 论文设定下成立且相对 5 个现代 baseline 具备优势证据, 再由 `stage_transition_decision` 生成 `validation_scale_to_pilot_paper_transition_decision`, 之后只能进入 `pilot_paper`; 不能直接进入 `full_paper`。`pilot_paper` 和 `full_paper` claim 仍分别由更低 FPR、更大样本和对应 checker 放行。如果 external baseline、内部消融、adaptive attack、replay/sketch、CI 或 artifact rebuild 在 validation_scale 阻断, 后续步骤不得用 paper 级运行补造缺失产物。`reviewer_evidence_index_builder` 必须晚于 `full_paper_result_checker`, 因为它只能索引已经由 checker 确认的 claims、tables、figures、reports 和 manifests, 不能在 claim 未通过前提前构造审稿证据。
