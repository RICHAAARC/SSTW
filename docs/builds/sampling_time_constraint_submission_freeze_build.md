...
在 `submission_freeze_preparation` 通过后, runner 会额外生成 readiness summary:

```text
artifacts/submission_readiness_summary.json
tables/submission_readiness_claim_table.csv
reports/submission_readiness_summary_report.md
```

该 summary 只从 `records/claim_audit_records.jsonl` 和 submission preparation decision 重建, 不手工新增 claim。当前允许的论文叙事边界为:

```text
main_submission_variant = SSTW-T
exploratory_variants = [SSTW-TC]
```
...
该 summary 的作用是指导论文写作与审稿材料整理, 不改变 B6 的降级结论。


---

## 17. Main tables rebuilt from governed records

`submission_freeze_preparation` runner 会从 `submission_stage_evidence_records.jsonl`、`claim_audit_records.jsonl` 和 `submission_readiness_summary.json` 重建论文主表候选:

```text
tables/submission_stage_evidence_main_table.csv
tables/submission_main_claim_table.csv
tables/submission_exploratory_boundary_table.csv
artifacts/submission_main_tables_manifest.json
reports/submission_main_tables_report.md
```

这些表格只作为 paper table candidates, 不新增 claim。主文表仅包含 `claim_scope = main` 且 `claim_status = supported` 的 claim。`SSTW-TC` 相关内容进入 exploratory boundary table, 其允许位置为 appendix 或 exploratory section。

---

## 18. Wan2.1 Flow Matching 主线要求

从本轮工程推进开始, SSTW-TC 的真实生成主线模型切换为:

```text
Wan-AI/Wan2.1-T2V-1.3B-Diffusers
```

LTX-Video 仍然可以作为工程预验证和回退 probe, 但不能单独支撑 “Flow Matching 生成轨迹中的状态空间水印” 主 claim。Wan2.1 主线结果必须同时满足:
...
为了证明 velocity / flow trajectory 确实参与水印同步, 采样 callback records 必须记录相邻 latent callback 位移形成的 flow velocity proxy:
...
```text
primary_flow_matching_model_ready = true
flow_velocity_proxy_ready = true
keyed_flow_velocity_alignment_gain_mean > baseline_flow_velocity_alignment_gain_mean
```

该证据只能说明采样轨迹中的状态更新参与同步, 仍不能替代最终 `TPR@FPR=0.01` formal evaluation。
