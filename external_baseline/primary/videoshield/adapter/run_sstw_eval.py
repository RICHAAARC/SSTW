"""把 VideoShield 官方实现适配到 SSTW external_baseline 正式比较协议。"""
...
ADAPTER_NAME = "videoshield"
ADAPTER_PATH = "external_baseline/primary/videoshield/adapter/run_sstw_eval.py"
ADAPTER_CONFIG = ModernBaselineCommandConfig(
    baseline_name=ADAPTER_NAME,
    baseline_family="in_generation_or_diffusion_video_watermark_baseline",
    adapter_path=ADAPTER_PATH,
    env_var="SSTW_VIDEOSHIELD_EVAL_COMMAND",
    default_source_script="external_baseline/primary/videoshield/source/run_sstw_eval.py",
    score_source="videoshield_official_adapter_output",
)


def adapter_status() -> dict[str, Any]:
    """返回 VideoShield adapter 的正式接入状态。
...
def build_score_records(run_root: str | Path, baseline_record: Mapping[str, Any]) -> list[dict[str, Any]]:
    """调用 VideoShield 官方命令并写出统一 comparison score records。"""
