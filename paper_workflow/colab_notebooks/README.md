...
| `generative_video_quality_scoring_colab.ipynb` | `generative_video_quality_scoring` | GPU 推荐 | `generative_video_generation_colab`, `motion_threshold_calibration_colab` | `generative_video_quality_scoring_colab` | 必须等 generation 和 motion threshold zip 完成。完成后可解锁机制后处理与 runtime attack。 |
| `sstw_mechanism_postprocess_colab.ipynb` | `sstw_mechanism_postprocess` | CPU 足够 | `generative_video_generation_colab`, `generative_video_quality_scoring_colab` | `sstw_mechanism_postprocess_colab` | 可在 quality scoring 完成后与 `runtime_attack_colab.ipynb` 并行。 |
| `runtime_attack_colab.ipynb` | `runtime_attack` | CPU 足够 | `generative_video_generation_colab`, `generative_video_quality_scoring_colab` | `runtime_attack_colab` | 可在 quality scoring 完成后与机制后处理并行。它产生 attacked videos, 后续 detection 和 baseline 依赖它。 |
| `runtime_detection_colab.ipynb` | `runtime_detection` | CPU 足够 | `generative_video_generation_colab`, `runtime_attack_colab` | `runtime_detection_colab` | 必须等 runtime attack 完成。完成后才允许启动 5 个主实验 baseline official reference Notebook。 |
...
| `revmark_formal_reference_colab.ipynb` | `external_baseline_formal_scoring / revmark` | GPU 推荐 | `generative_video_generation_colab`, `runtime_attack_colab`, `runtime_detection_colab` | `external_baseline_formal_reference_revmark` | 可与其它 baseline 并行。REVMark 属于后处理视频水印 baseline, 不应触发全量 unified scoring。 |
...
| `formal_comparison_scoring_colab.ipynb` | `formal_comparison_scoring` | CPU 足够 | `sstw_mechanism_postprocess_colab`, `runtime_detection_colab`, 5 个 `external_baseline_formal_reference_*` | `formal_comparison_scoring_colab` | 必须等待 5 个 baseline official reference zip 全部完成。它统一生成 `measured_formal` 和公平比较结果。 |
| `paper_evidence_postprocess_colab.ipynb` | `paper_evidence_postprocess` | CPU 足够 | 5 个 SSTW runtime 拆分阶段包, `motion_threshold_calibration_colab`, `formal_comparison_scoring_colab` | `paper_evidence_postprocess_colab` | 必须等待 formal comparison scoring 完成。它生成最终 gate 前的消融、CI、低 FPR、adaptive attack 和 leakage guard 等证据。 |
| `paper_gate_and_package_colab.ipynb` | `paper_gate_and_package` | CPU 足够 | 5 个 SSTW runtime 拆分阶段包, `motion_threshold_calibration_colab`, `formal_comparison_scoring_colab`, `paper_evidence_postprocess_colab` | `paper_gate_and_package_colab` | 每个 profile 的最终门禁。`pilot_paper` 还需要已通过的 `probe_paper` gate 包; `full_paper` 还需要已通过的 `pilot_paper` gate 包。 |

诊断 Notebook 不属于 paper workflow 的正式执行顺序:

| Notebook | 硬件要求 | 依赖关系 | 使用边界 |
|---|---|---|---|
| `wan21_flow_adapter_preflight_colab.ipynb` | GPU 必需 | 无正式阶段包依赖 | 只用于排查 Wan2.1 callback、time grid、sampler signature 和 latent trace 是否可捕获。通过后仍需回到正式主流程。 |
| `sampling_time_constraint_colab.ipynb` | GPU 必需 | 无正式阶段包依赖 | 只用于历史机制诊断和 sampling-time 约束排查, 不作为 paper profile 主流程入口。 |

并行运行必须遵守以下原则:

1. 只能并行运行没有直接前后依赖的 Notebook。任何 Notebook 的“必需前置阶段包”未完成时, 都不得提前启动。
2. 同一 profile 下, `sstw_mechanism_postprocess_colab.ipynb` 与 `runtime_attack_colab.ipynb` 是第一个推荐并行点。
3. 5 个主实验 baseline Notebook 是第二个推荐并行点, 但前提是 `runtime_detection_colab.ipynb` 已成功发布阶段包。
4. 不要在同一个 Google Drive 阶段归档目录中同时运行同一个 `workflow_profile + stage_package_id` 的两个实例, 否则最新 zip 选择、manifest 对齐和资源包发布可能产生竞态。
5. `formal_comparison_scoring_colab.ipynb`、`paper_evidence_postprocess_colab.ipynb` 和 `paper_gate_and_package_colab.ipynb` 必须串行执行。
6. `probe_paper -> pilot_paper -> full_paper` 必须按 gate 结果顺序推进。不同 profile 可以使用不同机器准备资源, 但不能在上一层 gate 未通过时把下一层结果解释为正式论文证据。
...
当 Wan2.1 无法加载、callback 捕获不到 latent、time grid 或 sampler signature 记录异常时, 可以先运行 `wan21_flow_adapter_preflight_colab.ipynb` 排查 adapter。排查通过后, 仍应回到当前推荐执行顺序的主流程。
