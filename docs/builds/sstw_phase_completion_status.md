...
该状态表示项目已经具备正式接入现代视频水印 baseline 的工程通道。当前仍未等价于完成真实 baseline 对比, 因为 6 个现代 baseline 需要用户在 Colab 或本地配置官方命令或 source 入口后, 才能在同一 run_root 上产出 `measured_formal` records。
