...
先在 mechanism_validation 中完成机制前置检查, 再让 validation_scale 以 FPR=10% 小样本全流程打通全部 paper 产物链路, 然后通过 validation_scale_to_pilot_paper_transition_decision 进入 pilot_paper; full_paper 仍需 pilot_paper、pilot_paper_to_full_paper_transition_decision 和 full_paper_result_checker 通过;
...
逐段执行, 并通过本项目完成 3 个主实验现代 baseline 的 clone / build / run / adapt / record。
...
3. 3 个主实验 modern external baseline formal reference Notebook 分别负责对应 baseline 的 source intake、clone / build / run / adapt、official bundle 生成, 并默认调用统一 runner 将已完成 bundle 转写为 `measured_formal` records。
4. `paper_gate_and_package_colab.ipynb` 负责恢复 3 个主实验 baseline official reference 阶段包, 重新执行全量统一转写和 self-containment 判定, 再执行内部消融、adaptive attack proxy、replay/sketch
   或 Claim-3 downgrade、CI、fixed-FPR gate、artifact rebuild 和 Drive package。

旧的通用 external baseline scoring Notebook 已删除。正式主流程应以 3 个主实验
...
在进入 `validation_scale` 前, 以较小成本验证 Claim-1、Claim-2 和部分 Claim-3 是否有成立迹象。该阶段不产生主论文表格, 也不直接决定是否进入 paper 级结果运行; `small_scale_mechanism_pilot_check` 通过后只允许进入 `validation_scale`。只有 `validation_scale` 通过并生成 `validation_scale_to_pilot_paper_transition_decision` 后, 才允许进入 `pilot_paper`。
...
本节描述真实生成式视频模型实验的实现 package, 不再定义独立主门禁。该 package 的输出由 `mechanism_validation`、`validation_scale`、`pilot_paper` 和 `full_paper_result_checker` 消费。

### 13.1 目标

在真实生成式视频模型上验证 SSTW 的轨迹观测、状态空间推断、检测协议、外部 baseline、内部消融、adaptive attack、replay/sketch、CI 和 artifact rebuild 是否形成完整 paper 机制闭环。该实现 package 应在 flow model adapter preflight、sampling-time constraint 和 small-scale 机制 pilot 检查之后使用, 但阶段放行以 `validation_scale`、`pilot_paper` 和 `full_paper` 主干门禁为准。
...
validation_scale_dataset: 用于在小样本规模验证 full_paper 运行链路、attack runner、baseline、消融和 claim gate 是否能够打通
...
在进入 validation_scale、pilot_paper 或 full_paper 前, 必须先运行 prompt observability audit。该 audit 只能使用视觉质量、运动可观测性和 prompt validity 字段, 禁止读取 `S_final`、`S_final_conservative` 或任何最终检测分数。
...
若现代视频水印 baseline 仍处于 `official_command_not_configured`、`manual_source_or_command_required` 或 `unsupported`, 则只能说明 baseline 接入尚未闭合, 不得进入 `pilot_paper` 结果运行, 也不得继续准备 `full_paper` claim。
...
4. 不跳过 `validation_scale`、`pilot_paper`、`full_paper_result_checker` 或三个轻量判定进入 full_paper。
...
这些跳转判定只能在 source gate PASS 后生成, 并由 target gate 消费。例如 `validation_scale_to_pilot_paper_transition_decision` 由 `pilot_paper_gate` 消费, 不得作为 `validation_scale` 自身 PASS 条件。
...
配置 3 个主实验现代 baseline 自包含 adapter 命令
可选执行 source clone
...
为了减少 Colab 中为 3 个主实验现代 baseline 重复编写 SSTW 外层 I/O wrapper 的成本, 项目提供统一 bridge:
...
当前项目提供 3 个主实验 repository official adapter:
...
真实生成前检查 3 个主实验 `SSTW_<BASELINE>_OFFICIAL_EVAL_COMMAND` 是否齐全。若用户希望完全自定义
...
validation_scale、pilot_paper 与 full_paper 的外部 baseline 流程不得停留在“只检查缺什么”。在
...
-> paper_gate_and_package_colab 重新聚合 3 个主实验 official reference 阶段包
```
...
6. 单 baseline Notebook 中的 comparison records 只用于即时自检。最终论文 gate 必须在
   `paper_gate_and_package_colab.ipynb` 恢复 3 个主实验 official reference 阶段包后重新运行
...
baseline 对比的 fail-closed 边界。严格门禁通过的前提仍然是 3 个主实验现代 baseline 最终都由本项目产出
