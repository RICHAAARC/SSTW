...
先在 mechanism_validation 中完成机制前置检查, 再让 probe_paper 以 FPR=10% 小样本论文闭合全部 paper 产物链路并验证主张候选, 然后通过 probe_paper_to_pilot_paper_transition_decision 进入 pilot_paper; full_paper 仍需 pilot_paper、pilot_paper_to_full_paper_transition_decision 和 full_paper_result_checker 通过;
...
3. 在 `probe_paper` 层, SSTW full method 必须在 `target_fpr=0.1` 下完成公平比较产物闭合和小样本论文闭合判定; 该层只支持 FPR=10% 小样本结论候选, 不支持外推为 `pilot_paper` 或 `full_paper` 结论。在 `pilot_paper` 层才可以报告 `TPR@FPR=0.01` 级别的 pilot 对比。
