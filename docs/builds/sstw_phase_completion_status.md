...
| `real_video_latent_transfer_check` | 部分完成 | VAE/视频链路模块和 runner 已存在。 | 真实视频 VAE 大规模低误报验证不足。 | 在 probe_paper 与 pilot_paper 前置闭合后补齐 real-video transfer validation。 | 影响 endpoint robustness 与视频链路可信度。 |
...
是否继续进入 `probe_paper`, 必须由 small_scale_mechanism_pilot_check 结果决定。
...
```text
primary_blocker: probe_paper_full_pipeline_not_completed
...
```text
probe_paper prompt / seed / attack manifest 规划
probe_paper full-pipeline runner / checker 增强
...
最新原生复跑已经满足上述标准。因此, 后续工作不应继续围绕“修复 pilot”展开, 而应进入 probe_paper 与论文级证据充分性构建。
...
generative video phase 新增 probe_paper 充分性矩阵
...
```text
1. 设计并执行 probe_paper full-pipeline rehearsal。
...
本次补强进一步降低“文档无法落地为 Codex 工程任务”的风险。当前项目阶段已经从 pilot 阻塞转为 probe_paper 阻塞:
...
最新 Google Drive 落盘结果显示, `small_scale_claim_pilot_gate` 已经通过。当前项目不再处于 small-scale pilot 阻塞状态, 而是进入 probe_paper 与论文级证据充分性构建阶段。
...
```text
probe_paper generative video probe planning
modern external baseline governed status records and adapter contracts
internal ablation matrix records
fixed-FPR and confidence interval reporter
flow-specific adaptive attack runner design
...
## 2026-06-23 probe_paper gate 工程推进状态

### 本次新增工程入口

为防止从 small-scale pilot 直接跳到 full_paper, 当前仓库新增 probe_paper gate:

```text
experiments/generative_video_model_probe/paper_profile_gate.py
configs/protocol/probe_paper_generative_probe.json
paper_workflow/notebook_utils/generative_video_model_probe_workflow.py::build_paper_profile_gate_command
paper_workflow/colab_notebooks/generative_video_generation_colab.ipynb SSTW_WORKFLOW_PROFILE = probe_paper
paper_workflow/colab_notebooks/generative_video_quality_scoring_colab.ipynb SSTW_WORKFLOW_PROFILE = probe_paper
paper_workflow/colab_notebooks/sstw_mechanism_postprocess_colab.ipynb SSTW_WORKFLOW_PROFILE = probe_paper
paper_workflow/colab_notebooks/runtime_attack_colab.ipynb SSTW_WORKFLOW_PROFILE = probe_paper
paper_workflow/colab_notebooks/runtime_detection_colab.ipynb SSTW_WORKFLOW_PROFILE = probe_paper
```
...
当前项目已经具备 probe_paper 的工程审计入口, 但尚未完成 probe_paper 真实运行。当前状态应表述为:

```text
small_scale_claim_pilot_gate: PASS
paper_profile_gate_checker: implemented
paper_profile_gate_decision: waiting_for_probe_paper_run
full_paper_allowed: false
submission_freeze_allowed: false
```
...
## 2026-06-23 probe_paper 后处理闭环工程推进

### 本次新增工程能力

在延后真实 GPU 复跑的前提下, 当前仓库继续补齐 probe_paper 运行后的自动后处理链路:
...
这些模块不运行 Wan2.1, 不生成 full_paper 主表, 也不把 validation proxy 伪装成最终论文 claim。它们的作用是让未来 `PROFILE = probe_paper` 复跑结束后, 以下产物自动闭环:
...
```text
paper_profile_gate_checker: implemented
probe_paper_postprocess_runners: implemented
probe_paper_real_gpu_run: not_yet_rerun
full_paper_allowed: false
submission_freeze_allowed: false
```
...
下一步是配置现代 baseline 官方命令或 source 入口, 保持 adaptive attack、replay/sketch 或 Claim-3 降级路径闭合, 然后执行 probe_paper 真实复跑。现代 baseline formal records 不得推迟到 pilot_paper 后补。
...
当前项目允许短期实现 `Claim-3 downgrade gate` 以保护 probe_paper 流程和 claim 边界, 但这不表示 Claim-3 已经完成。项目最终若要把 replay posterior、authenticated sketch 和 wrong replay 分离作为强创新点, 必须实现并通过 `replay/sketch gate`。

当前阶段性判断为:

```text
claim3_downgrade_gate: allowed_for_probe_paper_short_term
...
当前已补齐 probe_paper 的 adaptive attack 硬阻断 runner。该 runner 从 runtime detection records 生成 Flow-specific adaptive attack proxy records, 覆盖 scheduler/time grid mismatch、wrong sampler replay、endpoint-path decoupling、path response cancellation 和 trajectory sketch replacement attempt。
...
该更新只能让 probe_paper gate 获得 governed adaptive attack records, 不能支撑 full_paper 中的 `robust_to_flow_specific_adaptive_attacks` 强主张。

## 2026-06-24 probe_paper hard-blocker workflow 接入

当前 generative video Colab workflow 已接入 adaptive attack validation proxy 与 Claim-3 downgrade gate。probe_paper artifact rebuild dry-run 也已将两类新增产物纳入 required inputs / outputs 检查。
...
probe_paper_real_gpu_run: pending_user_colab_rerun
```

下一步是执行本地 probe_paper 工程闭环核验。若本地测试与 harness 均通过, 项目将等待用户在 Colab 中执行真实 Wan2.1 GPU probe_paper 复跑。

## 2026-06-24 formal motion exclusion gate bug 修复

已修复 small-scale / probe_paper 衔接中的一个 gate 判定问题: 原逻辑把任何一个 formal motion consistency 失败样本都解释为 `formal_motion_claim_ready` 缺失, 即使该样本已经被 formal motion filter 从 claim eligible set 中排除, 且剩余样本仍满足 prompt / seed 覆盖要求。
...
该变更使 probe_paper 结果中“23 个 eligible 样本 + 1 个 formal motion exclusion”能够被正确解释: 若 8 个 prompt 均仍至少保留 2 个 seed, 则 `small_scale_claim_pilot_gate_passed` 不应再因为已剔除样本而失败。后续真实 GPU 复跑仍需重新生成并落盘 artifacts, 本地代码层面已经补充对应回归测试。

## 2026-06-24 replay/sketch gate validation proxy 工程推进

在 probe_paper gate 已通过后, 项目已开始进入 replay/sketch、现代 external baseline 和真实内部消融的后续建设阶段。第一项推进为 replay/sketch gate validation proxy:
...
该状态说明 probe_paper 工程流程可以继续向 pilot_paper gate 推进, 但 Claim-3 仍保持降级边界。后续还需要把 validation proxy 升级为 full_paper 级 authenticated replay 和 replay negative FPR 审计。


## 2026-06-24 pilot_paper FPR=0.01 工程入口

当前 `generative_video_model_probe` 已新增 `pilot_paper` 语义层级, 用于在 probe_paper 之后执行小样本论文级结果包。该层级不是 workflow-only pilot, 而是小规模跑代表性 paper 协议并产出 pilot 级论文结果。
...
因此下一步真实 GPU 复跑顺序仍是先 `PROFILE = probe_paper`, 再 `PROFILE = pilot_paper`。如果 `pilot_paper` 运行后 baseline 或消融 records 未覆盖同批 held-out test trace, gate 会失败, 不能报告 pilot 级 `TPR@FPR=0.01`。
...
```text
probe_paper_requirement: upgraded_to_small_sample_full_pipeline_gate
probe_paper_code_gate: implemented_with_full_baseline_hard_blockers
probe_paper_real_gpu_run: pending_after_official_baseline_command_configuration
pilot_paper_allowed: false_until_probe_paper_full_pipeline_pass
full_paper_allowed: false_until_probe_paper_probe_paper_pilot_paper_and_full_paper_checker_pass
```

该更新的核心含义是: `pilot_paper` 只能执行 paper 级结果运行和报告 pilot 级结论, 不能再承担补现代 baseline、补内部消融、补 replay/sketch、补 CI 或补 artifact rebuild 的职责。若这些内容在 `probe_paper` 中没有闭合, 下一步必须修复 probe_paper 机制, 不能进入 `pilot_paper`。
...
probe_paper_modern_baseline_hard_gate: implemented
...
probe_paper_single_notebook_gate_test: removed_split_workflow_required
probe_paper_run_through_test_without_fake_claim: removed_with_monolithic_notebook
```

该变更解决的问题是: `probe_paper`、`probe_paper`、`pilot_paper` 和 `full_paper` 不再依赖 Notebook 中多处硬编码路径、样本数量或 profile 集合。切换运行层级时, 用户只需要设置:
...
probe_paper_missing_modern_command_preflight: still_hard_block
```
...
该更新没有绕过 probe_paper 门禁。只有当本项目在 Colab 或等价受治理环境中完成 clone / build / run / adapt / record, 准备权重、编写真实 wrapper, 并显式设置 5 个主实验 `SSTW_<BASELINE>_EVAL_COMMAND` 后, 现代 baseline 才能产出 `measured_formal` records。仅存在 URL、clone plan 或 command 模板不能支撑 baseline comparison claim。

## 2026-06-25 现代 baseline repository bridge command 接入

为推进 `probe_paper` 正式门禁跑通, 当前工程新增统一官方命令桥接器:
...
probe_paper_split_notebook_path: runnable_after_official_inner_commands_configured
modern_external_baseline_measured_formal_results: still_pending_real_colab_official_commands
```

该状态表示 probe_paper 的工程阻断已经从“缺 SSTW 外层 wrapper”收敛为“需要在 Colab 或等价受治理环境中为 5 个主实验官方 baseline 完成项目内 clone / build / run / adapt / record, 并配置真实官方命令和权重”。如果这些内部官方命令输出 score JSON, `external_baseline_runner` 会把结果转换为 `measured_formal` records, 并由 `external_baseline_execution_manifest.json` 绑定证据路径。

补充约束: 显式设置的 `SSTW_<BASELINE>_EVAL_COMMAND` 优先级高于默认 bridge 模板。因此,
probe_paper 正式门禁同时支持两条可跑通路径: `repository bridge + SSTW_<BASELINE>_OFFICIAL_EVAL_COMMAND`
...
为满足 `probe_paper` 严格正式门禁对 5 个主实验现代视频水印 baseline 的统一接入要求, 当前仓库新增 fail-closed 的 repository official adapter 入口:
...
因此, 当前阶段可以表述为: 5 个主实验现代 baseline 的 SSTW command adapter 和 repository official adapter 已经完成工程接入; 严格正式门禁仍必须通过 Colab 或等价受治理环境中的项目内 clone / build / run / adapt / record 证明这些 adapter 能基于官方源码、权重或项目生成的官方结果缓存写出 `measured_formal` records。若缺少这些官方输入, probe_paper 会失败, 且该失败是正确的 fail-closed 行为。
...
probe_paper_split_baseline_bundle_stage: integrated
...
`measured_formal`。因此严格 probe_paper 通过条件没有降低: 5 个主实验现代 baseline
...
不再接受外部补交 result bundle、手写 JSON、NPZ 分数文件、论文表格数字或 SSTW proxy 分数作为主表 baseline 证据。若某个现代 baseline 无法在项目流程内获得官方权重、checkpoint、maintained info 或运行环境, checker 必须写出 `non_runnable_with_governed_reason`, 并阻断 `probe_paper` PASS。

当前操作手册执行闭环在本轮已补齐 probe_paper 相关 builder 与轻量判定实现, 仍有以下缺口:

```text
paper_profile_gate_figure.json: implemented_in experiments/generative_video_model_probe/probe_paper_artifact_package.py
probe_paper_package_manifest.json: implemented_in experiments/generative_video_model_probe/probe_paper_artifact_package.py
...
external_baseline project_clone / project_build / project_run / project_adapt / project_record 已有轻量 checker, 但真实 Colab measured_formal 运行证据仍需在 probe_paper GPU run 中产出
```

当前阶段结论:

```text
probe_paper_definition_updated: true
full_paper_protocol_config_registered: true
external_baseline_self_contained_output_required: true
probe_paper_real_gpu_run: pending
pilot_paper_allowed: false_until_probe_paper_pass
full_paper_allowed: false_until_probe_paper_probe_paper_pilot_paper_and_full_paper_checker_pass
```
...
其中 `stage_transition_decision` 已拆成阶段明确的跳转判定: `probe_paper_to_pilot_paper_transition_decision` 与 `probe_paper_to_pilot_paper_transition_decision`、`pilot_paper_to_full_paper_transition_decision` 和 `full_paper_to_submission_freeze_transition_decision`。这些判定只能在 source gate 已 PASS 后生成, 并由 target gate 消费, 不得反向作为 source gate 自身 PASS 条件。
