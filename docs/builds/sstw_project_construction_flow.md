...
其中 `probe_paper` 重新定义为“target_fpr=0.1 小样本全流程打通验证”, 是进入 `probe_paper` 前的完整协议打通层。它必须使用 `configs/protocol/probe_paper_generative_probe.json` 中 `target_fpr=0.1` 指定的口径跑通与论文协议同构的全部产物链路, 但不在该层支持正式效果主张。`replay_and_authenticated_sketch_gate`、`flow_specific_adaptive_attack_gate`、external baseline、internal ablation、CI reporter、artifact rebuild 和 claim audit 都必须在 `probe_paper` 中形成可落盘、可检查、可失败闭环, 不得推迟到 `probe_paper`、`pilot_paper` 或 `full_paper` 后再补。`probe_paper` 必须能在小样本规模上产出 paper 相关的全部 governed artifact 类型: generation / detection records、主方法 measured_formal 结果、完整外部 baseline 对比、内部消融、46 个 runtime attack、11 个 non-runtime/adaptive 协议、replay/sketch 或受治理 Claim-3 downgrade、fixed-FPR CI、tables、figures、reports、package manifest 和 claim audit。只有 `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后, 才允许进入 `pilot_paper`; `probe_paper` 使用 pilot_paper 级样本结构在同一 FPR=10% 口径下判断 SSTW 是否成立以及是否相对 5 个现代 external baseline 具备优势证据。只有 `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后, 才允许进入 `pilot_paper`; `probe_paper` 和 `probe_paper` 都不能直接允许进入 `full_paper`。`full_paper` 仍必须等待 `pilot_paper`、`pilot_paper_to_full_paper_transition_decision`、`full_paper_result_checker`、CI、claim audit、artifact rebuild 和 submission freeze 相关门禁通过。`pilot_paper` 的定位是在 `probe_paper` 通过后, 使用代表性 paper 协议执行 FPR=1% 小规模结果运行并报告 pilot 级 `TPR@FPR=0.01`; 它不应再承担补机制、补 baseline 或补消融的职责。
...
先在 mechanism_validation 中完成机制前置检查, 再让 probe_paper 以 FPR=10% 小样本全流程打通全部 paper 产物链路, 然后通过 probe_paper_to_pilot_paper_transition_decision 进入 pilot_paper, 再由 pilot_paper 以 FPR=10% 小样本论文闭合验证证明主张可写, 最后通过 probe_paper_to_pilot_paper_transition_decision 进入 pilot_paper; full_paper 仍需 pilot_paper、pilot_paper_to_full_paper_transition_decision 和 full_paper_result_checker 通过;
...
```text
workflow_profile: motion_calibration / probe_paper / probe_paper / pilot_paper / full_paper
result_tier
runtime_profile
...
本节替代旧的 `small_scale_claim_pilot_gate` 主门禁表述。旧名称只作为历史文件名、历史 artifact 或代码兼容名出现, 不再作为主干门禁。该检查属于 `mechanism_validation` 的子检查, 不能单独放行 `pilot_paper` 或 `full_paper`。

### 12.1 目标

在进入 `probe_paper` 前, 以较小成本验证 Claim-1、Claim-2 和部分 Claim-3 是否有成立迹象。该阶段不产生主论文表格, 也不直接决定是否进入 paper 级结果运行; `small_scale_mechanism_pilot_check` 通过后只允许进入 `probe_paper`。只有 `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后, 才允许进入 `pilot_paper`; 只有 `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后, 才允许进入 `pilot_paper`。
...
本节描述真实生成式视频模型实验的实现 package, 不再定义独立主门禁。该 package 的输出由 `mechanism_validation`、`probe_paper`、`probe_paper`、`pilot_paper` 和 `full_paper_result_checker` 消费。

### 13.1 目标

在真实生成式视频模型上验证 SSTW 的轨迹观测、状态空间推断、检测协议、外部 baseline、内部消融、adaptive attack、replay/sketch、CI 和 artifact rebuild 是否形成完整 paper 机制闭环。该实现 package 应在 flow model adapter preflight、sampling-time constraint 和 small-scale 机制 pilot 检查之后使用, 但阶段放行以 `probe_paper`、`probe_paper`、`pilot_paper` 和 `full_paper` 主干门禁为准。
...
3. 在 `probe_paper` 层, SSTW full method 必须在 `target_fpr=0.1` 下完成小样本全流程打通和公平比较产物闭合, 但不支持正式效果主张; 在 `probe_paper` 层才可以报告 fpr=0.1 小样本论文闭合结论候选; 在 `pilot_paper` 层才可以报告 `TPR@FPR=0.01` 级别的 pilot 对比。
4. `probe_paper` gate 前必须已经生成同批小样本 test trace 的 external_baseline comparison records、内部消融 records、adaptive attack records、replay/sketch 或 Claim-3 downgrade records、CI report 和 artifact rebuild report。
5. 质量、运动和语义指标不能显示不可接受退化。
6. `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后只能进入 `pilot_paper`; `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后只能进入 `pilot_paper`; 不得直接进入 `full_paper`。full_paper claim 仍需 `pilot_paper_gate`、`pilot_paper_to_full_paper_transition_decision`、`full_paper_result_checker` 和轻量判定通过。若完整现代 baseline、内部消融或 replay/sketch 机制仍缺失, 只能报告阻断原因, 不能进入 paper 级结果运行。

### 13.6 probe_paper 作为 paper 级前小样本全流程打通层

`probe_paper` 的职责是证明 paper 级运行所需的全部机制和产物链路已经在小样本规模上闭合。它采用 probe_paper protocol config 中的 `target_fpr=0.1` 作为全流程打通口径, 目标是以最小成本提前发现 baseline、消融、attack、CI、artifact rebuild、claim audit 和 package 阻断, 但不在该层判断 SSTW 是否已具备可写论文的优势主张。`probe_paper` 在同一 FPR=10% 下使用更完整的小样本论文闭合协议, 才判断 SSTW 是否成立以及是否具备相对 5 个现代 external baseline 的优势证据。`pilot_paper` 在通过 probe_paper 后产出可报告的 FPR=1% pilot 级 paper 结果。
...
`replay/sketch gate` 是 Claim-3 的正式支撑门禁, 不能被 pilot 级 replay uncertainty、未认证 trajectory logging 或简单 replay proxy 替代。短期内可以使用 `Claim-3 downgrade gate` 解除 probe_paper 流程阻断, 但这只表示项目主动收缩 claim 边界, 不表示 replay/sketch 机制已经完成。
...
在进入 probe_paper、probe_paper、pilot_paper 或 full_paper 前, 必须先运行 prompt observability audit。该 audit 只能使用视觉质量、运动可观测性和 prompt validity 字段, 禁止读取 `S_final`、`S_final_conservative` 或任何最终检测分数。
...
-> comparison_decision
-> execution_manifest
-> probe_paper / probe_paper / pilot_paper / full_paper gate
```
...
`probe_paper` 是进入 `probe_paper` 前的小样本全流程打通门禁, 因此 external baseline 条件不得只要求两个显式同步 control。该阶段必须检查:
...
若现代视频水印 baseline 仍处于 `official_command_not_configured`、`manual_source_or_command_required` 或 `unsupported`, 则只能说明 baseline 接入尚未闭合, 不得进入 `probe_paper` 或 `pilot_paper` 结果运行, 也不得继续准备 `full_paper` claim。
...
4. 不跳过 `probe_paper`、`probe_paper`、`pilot_paper`、`full_paper_result_checker` 或三个轻量判定进入 full_paper。
...
| `mechanism_validation_checker` | 检查机制 records 与历史 small-scale pilot 记录是否足以进入 probe_paper | 否 |
| `validation_checker` | 检查 probe_paper 小样本全流程、baseline、攻击、消融、CI、artifact rebuild 和 claim audit 是否闭合 | 否, 只允许在生成 probe_paper_to_pilot_paper_transition_decision 后进入 probe_paper, 不能直接进入 pilot_paper 或 full_paper |
...
这些跳转判定只能在 source gate PASS 后生成, 并由 target gate 消费。例如 `probe_paper_to_pilot_paper_transition_decision` 由 `probe_paper_gate` 消费, `probe_paper_to_pilot_paper_transition_decision` 由 `pilot_paper_gate` 消费, 二者都不得作为各自 source gate 自身 PASS 条件。
...
full_paper 必须按以下顺序执行, 不得并行跳过 gate。`probe_paper` 承担 paper 级前的小样本全流程打通职责; `probe_paper` 承担 FPR=10% 小样本论文闭合职责; `pilot_paper` 承担 FPR=1% 小规模论文协议结果职责。三者均不能被 full_paper 运行后补造。

```text
1. verify_paper_profile_gate_passed
2. verify_probe_paper_gate_passed
3. verify_pilot_paper_gate_passed
4. freeze_full_paper_manifests
5. run_generation
6. run_attacks
7. run_replay_or_sketch_verification
8. run_detection
9. run_external_baselines
...
full_paper 目标包含 `TPR@FPR=0.001` 级别评估, 因此不能把一次超大规模运行作为首次端到端验证。Codex 必须在 full_paper 前至少完成 `probe_paper`, 用 probe_paper protocol config 指定的 target_fpr 小样本全流程打通验证证明数据、模型、攻击、检测、baseline、消融、统计和打包链路不会阻断; 随后通过 `probe_paper` 产出 FPR=10% 小样本论文闭合结果, 再通过 `pilot_paper` 产出由 pilot_paper protocol config 指定 target_fpr 的小规模论文协议结果。
...
probe_paper: FPR=10% 小样本全流程打通验证, 验证 baseline、ablation、adaptive attack、replay、CI 和 artifact rebuild
probe_paper: FPR=10% 小样本论文闭合验证, 判断结论是否可写
pilot_paper: FPR=1% 小规模代表性 paper 协议运行并产出 pilot 级论文结果
full_paper: 只在全部前置 gate 通过后执行
```

`method_mechanism_validation` 和 `probe_paper` 的结果只能用于排查链路和判断是否进入下一阶段, 不能替代 full_paper 主表。`probe_paper` 可以产出 FPR=10% 小样本论文闭合结论候选, `pilot_paper` 可以产出 pilot_paper 级论文结果, 但二者都不能替代 full_paper 主表。
...
当前操作手册已经定义 probe_paper、probe_paper、pilot_paper 和 full_paper 的语义顺序, 但仍必须显式保留以下工程缺口, 防止把文档完整误判为执行闭环已经完成:

```text
paper_profile_gate_figure_builder: implemented_in experiments/generative_video_model_probe/probe_paper_artifact_package.py
probe_paper_package_manifest_builder: implemented_in experiments/generative_video_model_probe/probe_paper_artifact_package.py
...
Notebook 必须在真实 GPU 生成前检查现代 baseline command 是否全部配置。若缺少任何一个 command, 应提前阻断, 不允许先生成缺 baseline 的混淆结果包。该规则属于项目特定写法, 目的是保证 `probe_paper` 作为 paper 级前小样本全流程打通层时已经能够产出完整 baseline comparison 结果。
...
probe_paper、probe_paper、pilot_paper 与 full_paper 的外部 baseline 流程不得停留在“只检查缺什么”。在
