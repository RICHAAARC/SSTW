...
| Claim-3 | Replay verification 可从攻击后视频恢复轨迹后验 | replay error 可控，model-side verification 成立 | 若稳定成立则作为强主贡献；否则作为限定设定贡献 |

Claim-1 与 Claim-2 必须成立，否则 SSTW 不应作为主论文投稿。Claim-3 若只在部分攻击下成立，论文应将 owner-side audit 作为高置信设定，将 replay verification 作为受限但有效的半白盒验证。
