...
| `stage_transition_decision` | 检查 validation_scale -> pilot_paper、pilot_paper -> full_paper、full_paper -> submission_freeze 的阶段跳转是否合法 | lightweight decision |
...
只有同时满足以下条件, 才能进入 `pilot_paper` 或继续准备 `full_paper` 后续流程。该 PASS 不直接允许 full_paper claim:

```text
target_fpr == 0.10
all_required_phase_decisions_exist
all_required_phase_decisions_passed_or_explicitly_downgraded
stage_transition_decision_ready
...
轻量判定不新增重型实验阶段, 只负责在阶段跳转时 fail closed。它们必须写出 machine-readable artifact, 并被 `validation_scale_readiness_gate`、`pilot_paper_gate`、`full_paper_result_checker` 或 `submission_package_freeze` 消费。
...
2. validation_scale_readiness_gate
3. stage_transition_decision、external_baseline_self_containment_decision 和 data_split_and_leakage_guard
4. modern_external_baseline_runner 的项目内 clone / build / run / adapt / record 与 measured_formal records
5. internal_ablation_matrix_runner
6. flow_specific_adaptive_attack_runner
7. replay_and_authenticated_sketch_gate 或受治理 Claim-3 downgrade gate
8. statistical_confidence_interval_reporter
9. pilot_paper_gate
10. reviewer_evidence_index_builder
11. full_paper_result_checker
```

原因是 `validation_scale` 是进入 paper 级运行前的小样本全流程打通层。所有 paper 相关机制必须先在 FPR=0.10 小样本口径下产出 governed records、tables、figures、reports、manifests 和 claim audit, 再进入 `pilot_paper` 或继续准备 `full_paper` 后续流程; `full_paper` claim 仍由 `full_paper_result_checker` 放行。如果 external baseline、内部消融、adaptive attack、replay/sketch、CI 或 artifact rebuild 在 validation_scale 阻断, 后续步骤不得用 paper 级运行补造缺失产物。
