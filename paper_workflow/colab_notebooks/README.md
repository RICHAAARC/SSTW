...
8. Colab 和普通 GPU 服务器都安装 `requirements/paper_runtime_lock.txt`, 并由服务器 CLI 读取 `requirements/paper_runtime_environment_lock.json` 验证依赖、CUDA、GPU、代码 commit 与模型 revision。预检失败时不得执行任何正式 stage。
