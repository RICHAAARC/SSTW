...
其中 `validation_scale` 重新定义为“target_fpr=0.1 小样本全流程打通验证”, 是进入 `probe_paper` 前的完整协议打通层。它必须使用 `configs/protocol/validation_scale_generative_probe.json` 中 `target_fpr=0.1` 指定的口径跑通与论文协议同构的全部产物链路, 但不在该层支持正式效果主张。`replay_and_authenticated_sketch_gate`、`flow_specific_adaptive_attack_gate`、external baseline、internal ablation、CI reporter、artifact rebuild 和 claim audit 都必须在 `validation_scale` 中形成可落盘、可检查、可失败闭环, 不得推迟到 `probe_paper`、`pilot_paper` 或 `full_paper` 后再补。`validation_scale` 必须能在小样本规模上产出 paper 相关的全部 governed artifact 类型: generation / detection records、主方法 measured_formal 结果、完整外部 baseline 对比、内部消融、46 个 runtime attack、11 个 non-runtime/adaptive 协议、replay/sketch 或受治理 Claim-3 downgrade、fixed-FPR CI、tables、figures、reports、package manifest 和 claim audit。只有 `validation_scale` 通过并生成 `validation_scale_to_probe_paper_transition_decision` 后, 才允许进入 `probe_paper`; `probe_paper` 使用 pilot_paper 级样本结构在同一 FPR=10% 口径下判断 SSTW 是否成立以及是否相对 5 个现代 external baseline 具备优势证据。只有 `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后, 才允许进入 `pilot_paper`; `validation_scale` 和 `probe_paper` 都不能直接允许进入 `full_paper`。`full_paper` 仍必须等待 `pilot_paper`、`pilot_paper_to_full_paper_transition_decision`、`full_paper_result_checker`、CI、claim audit、artifact rebuild 和 submission freeze 相关门禁通过。`pilot_paper` 的定位是在 `probe_paper` 通过后, 使用代表性 paper 协议执行 FPR=1% 小规模结果运行并报告 pilot 级 `TPR@FPR=0.01`; 它不应再承担补机制、补 baseline 或补消融的职责。
...
先在 mechanism_validation 中完成机制前置检查, 再让 validation_scale 以 FPR=10% 小样本全流程打通全部 paper 产物链路, 然后通过 validation_scale_to_probe_paper_transition_decision 进入 probe_paper, 再由 probe_paper 以 FPR=10% 小样本论文闭合验证证明主张可写, 最后通过 probe_paper_to_pilot_paper_transition_decision 进入 pilot_paper; full_paper 仍需 pilot_paper、pilot_paper_to_full_paper_transition_decision 和 full_paper_result_checker 通过;
...
```text
workflow_profile: motion_calibration / validation_scale / probe_paper / pilot_paper / full_paper
result_tier
runtime_profile
...
```text
full_paper_run_allowed: 只表示允许启动 full_paper 规模结果生产, 需要 validation_scale_gate、validation_scale_to_probe_paper_transition_decision、probe_paper_gate、probe_paper_to_pilot_paper_transition_decision、pilot_paper_gate、pilot_paper_to_full_paper_transition_decision 和 full_paper 运行前置门禁通过。
...
在进入 `validation_scale` 前, 以较小成本验证 Claim-1、Claim-2 和部分 Claim-3 是否有成立迹象。该阶段不产生主论文表格, 也不直接决定是否进入 paper 级结果运行; `small_scale_mechanism_pilot_check` 通过后只允许进入 `validation_scale`。只有 `validation_scale` 通过并生成 `validation_scale_to_probe_paper_transition_decision` 后, 才允许进入 `probe_paper`; 只有 `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后, 才允许进入 `pilot_paper`。
...
若 small-scale 机制 pilot 检查失败, 不应进入 `validation_scale`, 更不能进入 paper 级结果运行。应根据失败原因回退:
...
本节描述真实生成式视频模型实验的实现 package, 不再定义独立主门禁。该 package 的输出由 `mechanism_validation`、`validation_scale`、`probe_paper`、`pilot_paper` 和 `full_paper_result_checker` 消费。

### 13.1 目标

在真实生成式视频模型上验证 SSTW 的轨迹观测、状态空间推断、检测协议、外部 baseline、内部消融、adaptive attack、replay/sketch、CI 和 artifact rebuild 是否形成完整 paper 机制闭环。该实现 package 应在 flow model adapter preflight、sampling-time constraint 和 small-scale 机制 pilot 检查之后使用, 但阶段放行以 `validation_scale`、`probe_paper`、`pilot_paper` 和 `full_paper` 主干门禁为准。
...
3. 在 `validation_scale` 层, SSTW full method 必须在 `target_fpr=0.1` 下完成小样本全流程打通和公平比较产物闭合, 但不支持正式效果主张; 在 `probe_paper` 层才可以报告 fpr=0.1 小样本论文闭合结论候选; 在 `pilot_paper` 层才可以报告 `TPR@FPR=0.01` 级别的 pilot 对比。
...
6. `validation_scale` 通过并生成 `validation_scale_to_probe_paper_transition_decision` 后只能进入 `probe_paper`; `probe_paper` 通过并生成 `probe_paper_to_pilot_paper_transition_decision` 后只能进入 `pilot_paper`; 不得直接进入 `full_paper`。full_paper claim 仍需 `pilot_paper_gate`、`pilot_paper_to_full_paper_transition_decision`、`full_paper_result_checker` 和轻量判定通过。若完整现代 baseline、内部消融或 replay/sketch 机制仍缺失, 只能报告阻断原因, 不能进入 paper 级结果运行。

### 13.6 validation_scale 作为 paper 级前小样本全流程打通层

`validation_scale` 的职责是证明 paper 级运行所需的全部机制和产物链路已经在小样本规模上闭合。它采用 validation_scale protocol config 中的 `target_fpr=0.1` 作为全流程打通口径, 目标是以最小成本提前发现 baseline、消融、attack、CI、artifact rebuild、claim audit 和 package 阻断, 但不在该层判断 SSTW 是否已具备可写论文的优势主张。`probe_paper` 在同一 FPR=10% 下使用更完整的小样本论文闭合协议, 才判断 SSTW 是否成立以及是否具备相对 5 个现代 external baseline 的优势证据。`pilot_paper` 在通过 probe_paper 后产出可报告的 FPR=1% pilot 级 paper 结果。
...
`replay/sketch gate` 是 Claim-3 的正式支撑门禁, 不能被 pilot 级 replay uncertainty、未认证 trajectory logging 或简单 replay proxy 替代。短期内可以使用 `Claim-3 downgrade gate` 解除 validation_scale 流程阻断, 但这只表示项目主动收缩 claim 边界, 不表示 replay/sketch 机制已经完成。
...
validation_scale_dataset: 用于在小样本规模验证 full_paper 运行链路、attack runner、baseline、消融和 gate 产物是否能够打通; 正式 fpr=0.1 小样本论文闭合由 probe_paper_dataset 承担
...
在进入 validation_scale、probe_paper、pilot_paper 或 full_paper 前, 必须先运行 prompt observability audit。该 audit 只能使用视觉质量、运动可观测性和 prompt validity 字段, 禁止读取 `S_final`、`S_final_conservative` 或任何最终检测分数。
...
-> comparison_decision
-> execution_manifest
-> validation_scale / probe_paper / pilot_paper / full_paper gate
```
...
4. 不跳过 `validation_scale`、`probe_paper`、`pilot_paper`、`full_paper_result_checker` 或三个轻量判定进入 full_paper。
...
| `validation_checker` | 检查 validation_scale 小样本全流程、baseline、攻击、消融、CI、artifact rebuild 和 claim audit 是否闭合 | 否, 只允许在生成 validation_scale_to_probe_paper_transition_decision 后进入 probe_paper, 不能直接进入 pilot_paper 或 full_paper |
...
这些跳转判定只能在 source gate PASS 后生成, 并由 target gate 消费。例如 `validation_scale_to_probe_paper_transition_decision` 由 `probe_paper_gate` 消费, `probe_paper_to_pilot_paper_transition_decision` 由 `pilot_paper_gate` 消费, 二者都不得作为各自 source gate 自身 PASS 条件。
...
validation_scale_to_probe_paper_transition_decision_passed
...
full_paper 必须按以下顺序执行, 不得并行跳过 gate。`validation_scale` 承担 paper 级前的小样本全流程打通职责; `probe_paper` 承担 FPR=10% 小样本论文闭合职责; `pilot_paper` 承担 FPR=1% 小规模论文协议结果职责。三者均不能被 full_paper 运行后补造。
...
full_paper 目标包含 `TPR@FPR=0.001` 级别评估, 因此不能把一次超大规模运行作为首次端到端验证。Codex 必须在 full_paper 前至少完成 `validation_scale`, 用 validation_scale protocol config 指定的 target_fpr 小样本全流程打通验证证明数据、模型、攻击、检测、baseline、消融、统计和打包链路不会阻断; 随后通过 `probe_paper` 产出 FPR=10% 小样本论文闭合结果, 再通过 `pilot_paper` 产出由 pilot_paper protocol config 指定 target_fpr 的小规模论文协议结果。
...
`method_mechanism_validation` 和 `validation_scale` 的结果只能用于排查链路和判断是否进入下一阶段, 不能替代 full_paper 主表。`probe_paper` 可以产出 FPR=10% 小样本论文闭合结论候选, `pilot_paper` 可以产出 pilot_paper 级论文结果, 但二者都不能替代 full_paper 主表。
...
```text
validation_scale_readiness_gate
pilot_paper_gate
...
当前操作手册已经定义 validation_scale、probe_paper、pilot_paper 和 full_paper 的语义顺序, 但仍必须显式保留以下工程缺口, 防止把文档完整误判为执行闭环已经完成:
...
validation_scale_to_probe_paper_transition_decision: implemented_in scripts/check_results/stage_transition_decision.py
...
validation_scale、probe_paper、pilot_paper 与 full_paper 的外部 baseline 流程不得停留在“只检查缺什么”。在
