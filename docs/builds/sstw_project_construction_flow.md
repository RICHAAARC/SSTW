...
full_validation_dataset: 用于验证 full_paper 运行链路、attack runner、baseline、消融和 claim gate 是否稳定
...
当前工程约定为 21 个 `pilot_paper` prompt、每个 prompt 8 个 seed, 其中 4 个 calibration seed 只用于冻结阈值, 4 个 test seed 只用于 held-out FPR / TPR 报告。`pilot_paper` 与 `full_paper` 的区别只允许是样本规模和统计置信度; 该结论仍不能外推为 `TPR@FPR=0.001` 或 full-paper 规模主表结果。
...
pilot_paper gate 与 full_paper_run 启动前除了实验协议外, 还必须检查:
