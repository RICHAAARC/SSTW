...
只有同时满足以下条件, `paper_profile_gate` 才能写出 PASS。进入 `probe_paper` 还必须在该 PASS 落盘后生成 `probe_paper_to_pilot_paper_transition_decision`; 进入 `pilot_paper` 还必须等 `probe_paper_gate` PASS 并生成 `probe_paper_to_pilot_paper_transition_decision`; 该 PASS 不直接允许 pilot_paper 或 full_paper claim:
...
`probe_paper` 通过后仍不得声明 `TPR@FPR=0.1`、`TPR@FPR=0.001` 或 full_paper 主表结论。它只能说明 paper 级结果生产流程已经小样本跑通。它是进入 `pilot_paper` 和后续 `full_paper` 流程的必要条件, 但不是进入任一论文主张层的充分条件; `full_paper` 仍必须等待 `probe_paper_gate`、`pilot_paper_gate`、`full_paper_result_checker`、CI、claim audit 和 artifact rebuild 全部通过。

`paper_profile_gate_decision == PASS` 落盘后, 才允许运行 `stage_transition_decision` 生成 `probe_paper_to_pilot_paper_transition_decision`。该跳转判定只决定是否允许进入 `probe_paper`, 不反向参与 `probe_paper` 自身 PASS 计算。
...
除满足 probe_paper 的全部产物闭合条件外, `probe_paper_gate` 还必须满足:
...
probe_paper_gate 读取 probe_paper_to_pilot_paper_transition_decision
