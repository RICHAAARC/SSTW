...
pilot_paper_dataset: 用于小样本论文级结果包, 与 full_paper 同构协议, 差异只在样本规模
...
pilot_paper_rehearsal: 使用中等规模真实生成结果验证 pilot_paper 级 TPR@FPR=0.01 是否可报告
...
`fpr01_pilot_rehearsal` 在当前项目中升级为 `pilot_paper`。它可以报告 pilot_paper 级 `TPR@FPR=0.01` 论文主张, 前提是它采用与 full-paper 同构的数据集构造方式:
