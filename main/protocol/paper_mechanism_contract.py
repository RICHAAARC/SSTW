...
def load_paper_mechanism_contract(path: str | Path) -> dict[str, Any]:
    """读取完整机制契约并检查禁止降级的基础约束。"""

    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if payload.get("claim3_downgrade_allowed") is not False:
        raise ValueError("完整论文机制必须显式禁止 Claim-3 降级")
...
def audit_paper_profile_mechanism_contract(
    ...
    for row in rows:
        ...
        checks = {
            ...
            "claim3_downgrade_allowed": row.get("claim3_downgrade_allowed") is False,
