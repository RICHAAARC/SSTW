...
>
> 本文只保留历史设计与失败边界，不再提供当前方法导航或执行授权。当前唯一权威入口：
>
...
> 本地 P0--P9 与 Colab 白名单接线，但在该历史时点尚未执行 GPU 机制 smoke；不构成方法有效性、
> 阶段推进或论文 claim。
...
当前正式实现不使用普通特征 logistic 回归冒充状态空间后验。对无水印假设 \(H_0\) 与水印假设 \(H_1\) 分别在 calibration split 拟合：

\[
h_t^{(k)}=A_k h_{t-1}^{(k)}+b_k+q_t^{(k)},\qquad
x_t=h_t^{(k)}+r_t^{(k)},\qquad k\in\{0,1\}.
\]

其中，\(x_t\) 是固定 replay 路径上逐 Flow phase 测得的 endpoint、velocity、path、path-endpoint consistency、replay LLR、coverage 和可靠性观测。为控制有限样本下的参数方差，当前实现使用对角协方差与对角状态转移；这属于通用的正则化线性高斯状态空间写法。所有矩统计量与转移残差统计按 source-video group 等权估计, 同一视频的多 key 或多 replay 只能增加簇内观测, 不能增加独立统计权重。项目特定设计在于观测由同一个 Flow Tubelet Key Code、固定 key-independent inversion 路径和候选-vs-null replay 假设检验共同构造。

### 8.5 Filtering、smoothing 与概率后验

每个假设分别执行 Kalman filtering 和 Rauch-Tung-Striebel smoothing，并计算完整序列的边际似然：

\[
\ell_k=\log p(x_{1:T}\mid H_k),\qquad
\Lambda=\ell_1-\ell_0.
\]

随后只在 source-video group cross-fitting 得到的组外 \(\Lambda\) 上拟合 Platt 校准：

\[
P(H_1\mid x_{1:T})=\sigma(a\Lambda+b).
\]

Platt 层仅负责把状态空间似然比映射为可靠概率，不负责替代动态模型。cross-fitting 按 H0/H1 分层分配完整 source-video group; 若任一记录不能获得严格组外似然比, calibration 直接失败, 不允许用全量模型回填。held-out test 只能读取冻结的 \(A_k\)、\(b_k\)、噪声协方差、Platt 参数和 fixed-FPR 阈值。单步 fallback 可以服务旧诊断或单元测试，但正式 paper gate 要求至少2个实测 phase，并同时验证 filtering 与 smoothing 状态。
...
| 层级 | 主张 | 必须成立条件 | 投稿写法 |
|---|---|---|---|
| Claim-1 | Flow velocity constraint 能形成低扰动 endpoint watermark | endpoint evidence 分离；同 GPU generator state 的无速度约束生成对照在同一完整方法冻结检测器下产生显著配对差值；质量不明显下降 | 必须作为主贡献 |
