...
fpr01_pilot_rehearsal: 使用中等规模真实生成结果验证 pilot 级 TPR@FPR=0.01 是否可报告
...
`smoke_rehearsal`、`pilot_rehearsal`、`fpr01_pilot_rehearsal` 和 `validation_rehearsal` 的结果只能用于排查链路和判断是否进入下一阶段, 不能替代 full_paper 主表。

`fpr01_pilot_rehearsal` 例外地可以报告 pilot 级 `TPR@FPR=0.01` 结论, 前提是它采用与 full-paper 同构的数据集构造方式:
...
当前工程约定为 21 个 `fpr01_pilot` prompt、每个 prompt 8 个 seed, 其中 4 个 calibration seed 只用于冻结阈值, 4 个 test seed 只用于 held-out FPR / TPR 报告。该结论仍不能外推为 `TPR@FPR=0.001` 或 full-paper 主表结果。
