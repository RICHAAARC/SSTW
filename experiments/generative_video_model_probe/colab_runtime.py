...
def run_colab_probe(
    ...
    *,
    ...
    generation_plan_override: list[Mapping[str, Any]] | None = None,
    velocity_config_by_plan_record_id: Mapping[
        str, VelocityFieldConstraintConfig
    ]
    | None = None,
    ...
    | None = None,
    prompt_orthogonal_injection_by_plan_record_id: Mapping[
        str, PromptOrthogonalInjectionConfig | None
    ]
    | None = None,
    ...
    main_plan = (
        [dict(item) for item in generation_plan_override]
        if generation_plan_override is not None
        else _build_generation_plan(prompt_suite, profile, model_id, cross_model_id)
    )
    ...
    if generation_plan_override is not None:
        if profile not in {
            ...
            raise ValueError(
                "generation_plan_override 只允许已注册的轻量诊断 profile"
            )
        ...
        elif profile == PREDICTIVE_TRAJECTORY_SYNCHRONIZATION_SMOKE_PROFILE:
            plan_id_field = "predictive_trajectory_plan_record_id"
            required_plan_count = 8
        else:
            plan_id_field = "prompt_orthogonal_plan_record_id"
            required_plan_count = 8
        plan_ids = [
            str(item.get(plan_id_field) or "")
            for item in plan
        ]
        if len(plan) != required_plan_count or any(
            not value for value in plan_ids
        ):
            raise ValueError(
                f"{profile} generation override 必须包含"
                f"{required_plan_count}条带 plan ID 的记录"
            )
        if len(set(plan_ids)) != len(plan_ids):
            raise ValueError(f"{profile} generation plan ID 必须唯一")
        configured_ids = set(velocity_config_by_plan_record_id or {})
        if configured_ids != set(plan_ids):
            raise ValueError(
                f"{profile} velocity config 必须精确覆盖全部 plan ID"
            )
        if profile == MINIMAL_SIGNED_TRAJECTORY_STATE_SPACE_SMOKE_PROFILE:
            carrier_ids = set(
                signed_trajectory_carrier_by_plan_record_id or {}
            )
        ...
        if profile == PROMPT_ORTHOGONAL_STATE_TRAJECTORY_SMOKE_PROFILE:
            carrier_ids = set(
                prompt_orthogonal_injection_by_plan_record_id or {}
            )
            if carrier_ids != set(plan_ids):
                raise ValueError(
                    "prompt-orthogonal injection config "
                    "必须精确覆盖全部 plan ID"
                )
    ...
    active_pipe_by_model: dict[str, Any] = (
        pipeline_cache if pipeline_cache is not None else {}
    )
    ...
    for index, item in enumerate(plan):
        ...
        strength_level_id = str(
            item.get("embedding_strength_level_id") or ""
        )
        strength_suffix = f"_{strength_level_id}" if strength_level_id else ""
        carrier_variant_id = str(
            item.get("trajectory_carrier_variant_id") or ""
        )
        carrier_suffix = (
            f"_{carrier_variant_id}" if carrier_variant_id else ""
        )
        trace_id = (
            f"trace_{index:04d}_{item['sample_role']}_"
            f"{item['method_variant']}{strength_suffix}{carrier_suffix}"
        )
        ...
        video_path = output_root / "videos" / (
            ...
            f"{item['sample_role']}_{item['method_variant']}"
            f"{strength_suffix}{carrier_suffix}.mp4"
        )
        started = time.time()
        try:
            ...
            plan_record_id = str(
                ...
                or item.get("prompt_orthogonal_plan_record_id")
                or ""
            )
            if (
                profile
                ...
                mechanism_config = FlowVelocityRuntimeMechanismConfig(
                    velocity_constraint_enabled=bool(
                        (
                            prompt_orthogonal_injection_by_plan_record_id
                            or {}
                        ).get(plan_record_id)
                        is not None
                    ),
                ...
                mechanism_config = (
                    ...
                    if item["trajectory_carrier_variant_id"]
            ...
            with FlowVelocityConstraintRuntime(
                ...
                velocity_config=(
                    velocity_config_by_plan_record_id[
                        plan_record_id
                    ]
                    if velocity_config_by_plan_record_id is not None
                    else VelocityFieldConstraintConfig()
                ),
                latent_layout=latent_layout,
                key_context=key_context,
                signed_trajectory_carrier_config=(
                    (
                        signed_trajectory_carrier_by_plan_record_id[
                            plan_record_id
                        ]
                    )
                ...
                prompt_orthogonal_master_key_text=(
                    ...
                    if (
                        prompt_orthogonal_injection_by_plan_record_id
                        is not None
                        and prompt_orthogonal_injection_by_plan_record_id.get(
                            plan_record_id
                        )
                        is not None
                    )
                ...
                prompt_orthogonal_injection_config=(
                    ...
                    if prompt_orthogonal_injection_by_plan_record_id
                    is not None
                    else None
                ),
                prompt_orthogonal_scheduler_float32_control=(
                    profile
                    == PROMPT_ORTHOGONAL_STATE_TRAJECTORY_SMOKE_PROFILE
                ),
            ) as velocity_runtime:
            ...
            step_stats = [
                {
                    ...
                    **(
                        {
                            "controlled_embedding_plan_record_id": item[
                                "controlled_embedding_plan_record_id"
                            ],
                            "embedding_strength_level_id": item[
                    ...
                    **(
                        {
                            "signed_trajectory_plan_record_id": item[
                    ...
                    **(
                        {
                            "prompt_orthogonal_plan_record_id": item[
                        ...
                        if item.get("prompt_orthogonal_plan_record_id")
                        else {}
                    ),
                    **record,
                }
            ...
            applied_step_count = sum(
                record.get("velocity_field_constraint_status")
                in {
                    "applied",
                    "prompt_orthogonal_state_trajectory_applied",
                }
            ...
            if (
                velocity_runtime.canonical_endpoint_latent is not None
                and profile
                != PROMPT_ORTHOGONAL_STATE_TRAJECTORY_SMOKE_PROFILE
            ):
        ...
        generation_record = {
            ...
            **model_provenance,
            ...
            "method_variant": item["method_variant"],
            "watermark_embedding_status": item["watermark_embedding_status"],
            "velocity_constraint_config_id": (
                f"controlled_embedding_lambda_max_{float(item['lambda_max']):g}"
                if item.get("controlled_embedding_plan_record_id")
            ...
            **(
                {
                    ...
                    "embedding_strength_multiplier": float(
                        item["embedding_strength_multiplier"]
                    ),
                    "lambda_max": float(item["lambda_max"]),
                    "velocity_norm_ratio_budget": float(
                        item["velocity_norm_ratio_budget"]
                    ),
                    "flow_energy_budget_ratio": float(
                        item["flow_energy_budget_ratio"]
                    ),
                }
                if item.get("controlled_embedding_plan_record_id")
                else {}
            ...
            **(
                {
                    ...
                    "signed_trajectory_dc_allocation": float(
                        item["signed_trajectory_dc_allocation"]
                    ),
            ...
            **(
                {
                    ...
                    "lambda_max": float(item["lambda_max"]),
                    "prompt_orthogonal_negative_prompt_text_hash": sha256(
                        str(
                            item.get("prompt_negative_text") or ""
                        ).encode("utf-8")
                    ).hexdigest(),
                }
                if item.get("prompt_orthogonal_plan_record_id")
            ...
            "watermark_key_derivation_id": (
                ...
                if profile
                == PROMPT_ORTHOGONAL_STATE_TRAJECTORY_SMOKE_PROFILE
                else WATERMARK_KEY_DERIVATION_ID
            ),
            ...
            "flow_phase_schedule_id": (
                ...
                else "first_harmonic_two_channel_weighted_centered"
                if carrier_variant_id
                == "prompt_orthogonal_state_trajectory"
    ...
    controlled_embedding_profile = (
        profile == CONTROLLED_EMBEDDING_STRENGTH_DIAGNOSTIC_PROFILE
    )
    ...
    prompt_orthogonal_profile = (
        profile == PROMPT_ORTHOGONAL_STATE_TRAJECTORY_SMOKE_PROFILE
    )
    lightweight_diagnostic_profile = (
        controlled_embedding_profile
        or signed_trajectory_profile
        or predictive_trajectory_profile
        or prompt_orthogonal_profile
    )
    successful_variants = {
    ...
    controlled_embedding_execution_ready = bool(
        controlled_embedding_profile
        and len(generation_records) == len(plan) == 16
        and success_count == 16
        and bool(trajectory_records)
    )
    signed_trajectory_execution_ready = bool(
        signed_trajectory_profile
        and len(generation_records) == len(plan) == 12
        and success_count == 12
        and bool(trajectory_records)
        and {
            "signed_balanced_ac",
    ...
    prompt_orthogonal_execution_ready = bool(
        prompt_orthogonal_profile
        and len(generation_records) == len(plan) == 8
        and success_count == 8
        and bool(trajectory_records)
        ...
        and all(
            int(record.get("sampling_constraint_applied_step_count") or 0)
            > 0
            for record in generation_records
            if record.get("trajectory_carrier_variant_id")
            == "prompt_orthogonal_state_trajectory"
        )
        and all(
            int(record.get("sampling_constraint_applied_step_count") or 0)
    ...
    generation_records = [
        with_flow_evidence_protocol_defaults(
            ...
            claim_support_status=(
                ...
                else "controlled_embedding_strength_diagnostic_only_not_paper_evidence"
                ...
                else "prompt_orthogonal_state_trajectory_smoke_only_not_paper_evidence"
                if prompt_orthogonal_profile
    ...
    external_records = (
        []
        if lightweight_diagnostic_profile
        else run_external_baseline_status(
            "configs/external_baselines/external_baselines.json"
        )
    )
    external_baseline_audit = (
        {
            "external_baseline_execution_status": (
                ...
                if predictive_trajectory_profile
                else "not_run_prompt_orthogonal_state_trajectory_smoke"
            ),
            "external_baseline_execution_allowed": False,
        }
        if lightweight_diagnostic_profile
        else audit_external_baseline_records(external_records)
    )
    ...
    if not lightweight_diagnostic_profile:
    ...
    decision = {
        ...
        "mechanism_decision": (
            ...
            if mechanism_validation_profile
                ...
                if controlled_embedding_execution_ready
                else "FAIL"
            )
            if controlled_embedding_profile
            else (
                "GENERATION_READY_NO_ATTACK_REPLAY_PENDING"
                if signed_trajectory_execution_ready
                else "FAIL"
            )
            if signed_trajectory_profile
            else (
            ...
            if predictive_trajectory_profile
                ...
                "GENERATION_READY_NO_ATTACK_REPLAY_PENDING"
                if prompt_orthogonal_execution_ready
                else "FAIL"
            )
            if prompt_orthogonal_profile
        ...
        "details": {
            "formal_claim_status": (
                ...
                if mechanism_validation_profile
                else "controlled_embedding_generation_only_no_attack_replay_pending"
                if controlled_embedding_profile
                ...
                if predictive_trajectory_profile
                else "prompt_orthogonal_generation_only_no_attack_replay_pending"
                if prompt_orthogonal_profile
            ...
            "external_baseline_comparison_status": (
                ...
                if lightweight_diagnostic_profile
                else "limitation_records_written"
            ),
            "external_baseline_execution_status": external_baseline_audit.get(
                "external_baseline_execution_status",
                "status_records_only",
            ),
            "gpu_name": gpu_name,
