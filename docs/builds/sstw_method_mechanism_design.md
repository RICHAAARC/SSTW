...
如果 `replay/sketch gate` 未通过, 方法机制仍可保留 Claim-1 与 Claim-2, 但 Claim-3 必须通过 `Claim-3 downgrade gate` 降级为 owner-side audit 或 exploratory replay analysis。该降级路径是 claim governance, 不是 replay/sketch 机制本身的替代实现。
...
| Claim-3 | Replay verification 可从攻击后视频恢复轨迹后验 | replay/sketch gate 通过, replay error 可控，model-side verification 成立 | 若 gate 未通过则必须降级为限定设定贡献 |

Claim-1 与 Claim-2 必须成立，否则 SSTW 不应作为主论文投稿。Claim-3 若只在部分攻击下成立，论文应将 owner-side audit 作为高置信设定，将 replay verification 作为受限但有效的半白盒验证。若 `replay/sketch gate` 未实现或未通过, 只能使用 `Claim-3 downgrade gate` 降级, 不得把 robust replay verification 写成强 supported claim。
