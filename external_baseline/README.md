...
2. 可选的官方 source clone:
   ...
   该开关只会对可 git clone 的 source URL 生效。不能 git clone 的论文页面、PDF 或 HTML source 必须由 source intake 写出受治理的不可运行原因, 不能通过人工补交结果绕过 measured_formal 产出链路。

3. 自动运行证据路径:

   项目 workflow 会把 clone / build / run / adapt / record 过程中产生的官方 baseline 运行日志、配置、输出 JSON、依赖快照或校验文件写入 `artifacts/external_baseline_execution_manifest.json`。这些路径只用于 provenance 审计, 不能替代 `metric_status: measured_formal` records。
...
`probe_paper` 是 FPR=10% 小样本论文闭合入口。因此在 `SSTW_WORKFLOW_PROFILE=probe_paper`、`SSTW_WORKFLOW_PROFILE=pilot_paper` 或 `SSTW_WORKFLOW_PROFILE=full_paper` 时, Colab notebook 会在 5 个主实验现代 baseline command 缺失时提前阻断, 避免先消耗 GPU 生成视频后才发现 baseline 主表无法闭合。`pilot_paper` 是 FPR=1% 小规模论文结果入口。
