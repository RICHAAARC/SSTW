...
def _load_video_generation_pipeline(model_id: str, torch_dtype: Any) -> Any:
    ...
    emit_progress_event("sampling_constraint_model_load", f"start | model={model_id}")
    if _model_family_from_id(model_id) == "diffusers_wan21_flow_matching_dit":
        with suppress_third_party_progress_output("sampling_constraint_model_import"):
            from diffusers import WanPipeline

        with suppress_third_party_progress_output("sampling_constraint_model_load"):
    ...
    emit_progress_event("sampling_constraint_model_load", f"finish | model={model_id} | pipeline_progress_bar={progress_bar_status}")
    return pipe
...
def run_sampling_constraint_colab_probe(
    ...
    for index, item in enumerate(plan):
        ...
        started = time.time()
        try:
            with suppress_third_party_progress_output("sampling_constraint_single_video_generation"):
                result = pipe(
