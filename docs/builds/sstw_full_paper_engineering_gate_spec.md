...
| 组件 | 作用 | 产物性质 |
|---|---|---|
| `validation_scale_readiness_gate` | 进入 paper 级运行前的小样本全流程打通门禁, 使用 validation_scale protocol config 指定的 target_fpr 验证完整论文产物链路是否可生成 | gate decision |
...
`validation_scale` 的正式定义是“小样本全流程打通验证”。它不是 full_paper 效果证明, 也不是只检查单个 runner 是否能启动的 smoke test。该 gate 的功能是作为 paper 级前的全流程打通层, 在 validation_scale protocol config 指定的 `target_fpr` 小样本口径下提前验证 `pilot_paper` 和 `full_paper` 会需要的产物类型是否都能由本项目自动生成。
